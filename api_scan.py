...
def scan_resources(url, driver_path=None, no_headless=False, wait_time=5):
    ...
    others = sorted([r for r in filtered if r not in js_list + css_list + api_list])
    ...
    print(f"\n\033[92m--- Other URLs [{len(others)}] ---\033[0m")
    for r in others:
        print(r)
        run_for_url(r)
